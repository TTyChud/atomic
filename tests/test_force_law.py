...
from atomic.numerics.force_law import force_law_levels
...
def test_powerlaw_degeneracy_breaks_off_p1():
    ...
    e_2s = s.counterfactual[1].energy.value
    e_2p = p.counterfactual[0].energy.value
...
def test_yukawa_reference_is_full_hydrogen_ladder_even_under_shortfall():
    ...
    assert len(res.reference.items) == 4
