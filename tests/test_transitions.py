...
def test_einstein_A_2p_to_1s():
    ...
    assert a == pytest.approx(6.27e8, rel=3e-3)
...
def test_selection_rule_forbids_delta_l_zero_and_two():
    assert oscillator_strength(1, 0, 2, 0).value == 0.0
    assert oscillator_strength(2, 0, 3, 2).value == 0.0
    assert einstein_A(2, 0, 1, 0).value == 0.0

def test_absorption_requires_upper_above_lower():
    with pytest.raises(ValueError):
        oscillator_strength(2, 1, 1, 0)
